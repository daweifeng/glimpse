...
export default function ConnectionContainer() {
  ...
  useEffect(() => {
    ...
    connection
      .connect(`${serverWsUrl}/ws?&userId=${userId}&username=${username}`)
      .then(async () => {
  ...
  if (snap.peerConnectionState === PeerConnectionState.Waiting) {
    return (
      <div className="text-center">
        ...
        <div>
          <button
            className="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded transition ease-in-out delay-150"
            onClick={() => {
  ...
  if (snap.peerConnectionState === PeerConnectionState.ReceivdRequest) {
    return (
      <div className="flex flex-col text-center">
        ...
        <button
          className="min-w-40 bg-green-500 hover:bg-green-700 text-white font-bold mt-2 py-2 px-4 rounded transition ease-in-out delay-150"
          onClick={handleApproveJoinRoom}
        >
          Allow
        </button>
        <button
  ...
  if (snap.peerConnectionState === PeerConnectionState.Disconnected) {
    return (
      <div className="text-center">
        ...
        <button
          className="min-w-40 w-full bg-blue-500 hover:bg-blue-700 text-white font-bold mt-2 py-2 px-4 rounded transition ease-in-out delay-150"
          onClick={() => router.push("/")}
        >
          Back To Home
        </button>
